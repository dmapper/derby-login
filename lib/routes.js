...
module.exports = function (options) {
  ...
  function parseError(err, userId) {
    var data = {};
    if (!err) {
      var data = {
        success: true,
        url: options.passport.successRedirect,
      };
        ...
        case 'login':
          ...
          passport.authenticate('local', options.passport, function(err, user, info) {
            ...
            auth.login(user, req, function(err) {
              return res.json(parseError(err, user.id));
            });
          })(req, res);
          break;
        ...
        case 'register':
          var done = function(err, user){
            ...
            if (options.confirmRegistration) {
              ...
              var data ={
                success: true,
                url: options.confirmRegistrationUrl
              };
...
//                console.log('callback end: ', conf, req.query, oldUser);
...
//              console.log('before', method, conf);
              res.cookie('_query', JSON.stringify(req.query));
