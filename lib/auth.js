...
function register(data, userId, model, req, res, done) {
  ...
  model.fetch($userQuery, $user, function(err) {
    ...
    options.sendmail.manualRegistration(email, {
      username: email,
      password: password,
      name: fullName
    });
  ...
  model.fetch($query, function (err) {
    ...
    $user.set('local.passwordHash', passwordHash, function (err) {
      ...
      var mailData = {
        ...
        to: email,
        subject: "Password Reset for " + options.site.name,
        text: "A new password for " + email + " has been reset to " + newPassword + (". Log in at " + options.site.domain),
        html: "A new password for <strong>" + email + "</strong> has been reset to <strong>" + newPassword+ ("</strong>. Log in at " + options.site.domain)
      };
