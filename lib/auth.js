...
function registerProvider($user, provider, profile, req, res, done) {
  ...
  if (user) {
    $user.set(provider, profile, function(err) {
      console.log('add provider to the existing profile!');
      if (err) return done(err);
      $user.set('timestamps.registered', +new Date(), callback)
    })
  } else if (provider === 'linkedin') {
    done("We could not find your account - You have to first register to create a new account and you can then associate your account with your linked-in account to login automatically with one click");
  } else {
    ...
    function addUser(profId, termId) {
      ...
      if (profId) {
        user.profId = profile.profId;
        user.profIdAccepted = true;
        delete profile.profId;
      }
      if (termId) {
        user.termId = profile.termId;
  ...
  model.fetch(fetches, function(err) {
    ...
    var wrapperFn = function() {
      ...
      if ($term) {
        var term = $term.get();
        term = term && term[0];
        registerWithProfKey(term, prof, profile, data, options, email, done)
      }
...
function registerWithProfKey(term, prof, profile, data, options, email, done) {
  ...
  if (!options.profkey || options.profkey.indexOf(data.profkey) === -1) {
    if (!prof) {
      return done({profkey: "Can't find professor with the profKey!"})
    }
    ...
    if (!emailExtCorrect) {
      return done({email: "Your email-extension is not in the prof email-extensions list!"})
    }
